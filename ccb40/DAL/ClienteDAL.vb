...
Namespace DAL
    Public Class ClienteDAL
        ...
        Public Sub Insere(modelo As Cliente)
            Try
                ...
                sql = "INSERT INTO clientes(nome,contato_funcao,contato_nome,cgc_cpf,inscr_estadual,endereco,cidade,estado,cep,telefone1,telefone2,telefone3,email,obs)" + "VALUES (@nome,@contato_funcao,@contato_nome,@cgc_cpf,@inscr_estadual,@endereco,@cidade,@estado,@cep,@telefone1,@telefone2,@telefone3,@email,@obs)"
                modelo.ID = bd.exeNonQuery(sql, p)
                bllLog.GravaLog("Cadastro", "Cliente", "Cadastrado", modelo.ID)
            Catch erro As Exception
                bllLog.GravaLog("Cadastro", "Cliente", "Erro ao Cadastrar", modelo.ID)
        ...
        Public Sub Altera(modelo As Cliente)
            Try
                ...
                sql = "UPDATE clientes SET nome=@nome,contato_funcao=@contato_funcao,contato_nome=@contato_nome," + "cgc_cpf=@cgc_cpf,inscr_estadual=@inscr_estadual,endereco=@endereco,cidade=@cidade,estado=@estado,cep=@cep," + "telefone1=@telefone1,telefone2=@telefone2,telefone3=@telefone3,email=@email,obs=@obs" + " WHERE id=@id;"
                bd.exeNonQuery(sql, p)
                bllLog.GravaLog("Cadastro", "Cliente", "Alterado", modelo.ID)
            Catch erro As Exception
                bllLog.GravaLog("Cadastro", "Cliente", "Erro ao Alterar", modelo.ID)
                Throw New Exception(erro.Message)
            End Try
        End Sub
        ...
        Public Sub Exclui(id As Integer)
            Try
                ...
                bd.exeNonQuery(sql, p)
                bllLog.GravaLog("Cadastro", "Cliente", "Excluído", id)
            Catch erro As Exception
                bllLog.GravaLog("Cadastro", "Cliente", "Erro ao Excluir", id)
                Throw New Exception(erro.Message)
            End Try
        End Sub
        ...
        Public Function Filtrar(valor As [String], Optional where As String = "") As DataTable
            ...
            Else
                ...
                End If
            End If
            sql = "SELECT id, nome, contato_nome, telefone1 FROM clientes " & sWhere & " ORDER BY nome"
        ...
        Private Function BancoParaModelo(registro As MySqlDataReader) As Cliente
